...
export function UpdatePosition({ toUpdate, setUpdate, setReload }: {
    toUpdate: Position
    setUpdate: Dispatch<SetStateAction<Position | undefined>>
    setReload: Dispatch<SetStateAction<boolean>>
}) {
    const [onProcess, setProcess] = useState(false);
    const [position, setPosition] = useState<CreatePositionType>({
        name: toUpdate.position,
        parent_position_id: toUpdate.parent_position_id ?? 0,
        is_unique: toUpdate.is_unique === 1 ? true : false
    });
    ...
    async function handleSubmit() {
        try {
            ...
            const payload: CreatePositionType =
            ...
            const data = await PositionService.updatePosition(payload, toUpdate.id);
            if (data) {
